...
- **`subscribe/`** — `IMessageHandler<in T>` (business logic, returns `bool`), `IMessageKeySelector<in T>` (optional ordering key), and `IMessageSubscriber` (`Subscribe()`/`Unsubscribe()` lifecycle). `SolaceSubscriber<T>` binds a client-acknowledged, guaranteed-delivery `IFlow` to the queue named in `IMessageSubscriberSettings.Queue`.
